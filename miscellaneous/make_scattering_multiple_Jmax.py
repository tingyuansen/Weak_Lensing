...
#----------------------------------------------------------------------------------------------------------
    ...
    for l in range(7):
        for j in range(7):
            print("L=", l+1, " ,J=", j+1)
            scattering = Scattering2D(J=j+1, shape=(training_x[0,:,:].shape), L=l+1, max_order=3)
            scattering.cuda()
...
#----------------------------------------------------------------------------------------------------------
            ...
            for i in range(len(Sx)):
                if i > 0:
                    ...
                    Sx_array = Sx[i]
            print("Sx_array_size=", Sx_array.shape)
            np.save("Sx_" + train_name + "ing_L=" + str(l+1) + "_J=" + str(j+1) + "_expected_" + str(ng) + ".npy", Sx_array)
